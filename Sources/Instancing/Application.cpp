...
void Application::UpdateWorldBuffers()
{
    ...
    world_buffer.world = DirectX::XMMatrixTranspose(DirectX::XMMatrixIdentity());
...
bool Application::SDLInit()
{
    ...
    m_SdlWindow = SDL_CreateWindow("DirectX - Instancing", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, window_flags);
...
void Application::CalculateFramesPerSecond()
{
    ...
    if (time > 1.0f)
    {
        ...
        auto title = "DirectX - Instancing - FPS: " + std::to_string(fps) + " (" + std::to_string(1000.0f / fps) + " ms)";
