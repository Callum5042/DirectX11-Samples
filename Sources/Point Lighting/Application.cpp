...
bool Applicataion::SDLInit()
{
    ...
    m_SdlWindow = SDL_CreateWindow("DirectX - Point Lighting", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, window_flags);
...
void Applicataion::CalculateFramesPerSecond()
{
    ...
    if (time > 1.0f)
    {
        ...
        auto title = "DirectX - Point Lighting - FPS: " + std::to_string(fps) + " (" + std::to_string(1000.0f / fps) + " ms)";
