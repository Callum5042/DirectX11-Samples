...
void Application::SetLightBuffer()
{
    DX::LightBuffer light_buffer = {};
    light_buffer.directionalLight.diffuse = DirectX::XMFLOAT4(0.6f, 0.6f, 0.6f, 1.0f);
    light_buffer.directionalLight.ambient = DirectX::XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
    light_buffer.directionalLight.specular = DirectX::XMFLOAT4(0.2f, 0.2f, 0.2f, 8.0f);
...
bool Application::SDLInit()
{
    ...
    m_SdlWindow = SDL_CreateWindow("DirectX - Normal Mapping", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, window_flags);
...
void Application::CalculateFramesPerSecond()
{
    ...
    if (time > 1.0f)
    {
        ...
        auto title = "DirectX - Normal Mapping - FPS: " + std::to_string(fps) + " (" + std::to_string(1000.0f / fps) + " ms)";
