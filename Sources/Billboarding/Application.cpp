...
void Application::UpdateWorldBuffer()
{
    ...
    world_buffer.cameraPosition = m_DxCamera->GetPosition();
...
bool Application::SDLInit()
{
    ...
    m_SdlWindow = SDL_CreateWindow("DirectX - Billboarding", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, window_flags);
...
void Application::CalculateFramesPerSecond()
{
    ...
    if (time > 1.0f)
    {
        ...
        auto title = "DirectX - Billboarding - FPS: " + std::to_string(fps) + " (" + std::to_string(1000.0f / fps) + " ms)";
