...
int Applicataion::Execute()
{
    ...
    while (e.type != SDL_QUIT)
    {
        ...
        else
        {
            ...
            // Render the model
            /*m_DxShader->UpdateWorldBuffer(m_DxModel->World, m_DxModel->Texture);
            m_DxModel->Render();*/

            // Render the floor
            m_DxShader->UpdateWorldBuffer(m_DxFloor->World, m_DxFloor->Texture);
            m_DxFloor->Render();

            // Render the light as a model for visualisation
            m_DxShader->UpdateWorldBuffer(m_DxDirectionalLight->World, m_DxDirectionalLight->Texture);
